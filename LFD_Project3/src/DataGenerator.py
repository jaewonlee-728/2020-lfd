...
# }
used_column_name = {
    'race_result': ['date', 'race_num', 'track_length', 'track_state', 'rank', 'lane', 'horse', 'weight', 'jockey', 'trainer', 'owner'],
    'jockey': ['date', 'jockey', 'age', 'debut', 'weight','race_count', 'first', 'second', '1yr_count', '1yr_first', '1yr_second'],
    'owner': ['date', 'owner', '1yr_count', '1yr_first', '1yr_second', '1yr_third', 'race_count', 'first', 'second', 'third'],
    'trainer': ['date', 'trainer', 'age', 'debut', 'race_count', 'first', 'second', '1yr_count', '1yr_first', '1yr_second'],
    'horse': ['date', 'horse', 'home', 'gender', 'age', 'class','race_count', 'first', 'second', '1yr_count', '1yr_first', '1yr_second'],
}
...
def load_data():
    ...
    # TODO: Make Features
    # race result
    new_race_df = preprocessing.race_result_features(df_dict['race_result'])
    df_dict['race_result'] = new_race_df

    # 1. horse
    new_horse_df = preprocessing.horse_features(df_dict['horse'])
    df_dict['horse'] = new_horse_df

    # 2. jockey
    new_jockey_df = preprocessing.jockey_features(df_dict['jockey'])
    df_dict['jockey'] = new_jockey_df

    # 3. owner
    new_owner_df = preprocessing.owner_features(df_dict['owner'])
    df_dict['owner'] = new_owner_df

    # 4. trainer
    new_trainer_df = preprocessing.trainer_features(df_dict['trainer'])
    df_dict['trainer'] = new_trainer_df
    ...
    # TODO after finish implementing preprocess functions
